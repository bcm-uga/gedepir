...
merge_hc = function(A,k,T=NULL,plot=NULL,unknown_est=NULL) {
  ...
  if(is.null(T)) hc=hclust(d=as.dist(1-cor(t(A))))
  
  else hc=hclust(d=dist(scale(t(T))))
  ...
  } else  bestk_clust=cutree(hc, k)
  
  print(c(plot, bestk_clust))
  dend <- as.dendrogram(hc)
  
  Amerge=data.matrix(do.call(rbind
                             , lapply(unique(bestk_clust)
                                      ,function(icol)
                                      {idx=bestk_clust==icol
                                      if(sum(idx)>1) return(apply(A[idx,]
                                                                  , 2
                                                                  , sum))
                                      else return(A[idx,])
                                      }
...
prepare_A <- function(A_r, A_est, T_ref) {
  ...
  if ( nrow(x = A_est) > K ) {
    ...
    if (sum(filteredout) > K) {
      ...
      tokeep=setdiff(which(filteredout), nrow(A_est)) 
      unchar = NULL
      if (nrow(A_est)-ncol(T_ref)>0) {unchar = sum(filteredout)}  #handle the case in which an uncharacterized cell type is estimated by a reference based method
      A_est =  merge_hc(A = A_est[filteredout,]
                        , k =  nb_clust
                        , T= T_ref[, tokeep]
                        , unknown_est = unchar
      )
